...
{block name='main'}
    ...
    <form action="{path_for name="info"}" method="POST">
        ...
        <div class="champs">
            ...
            {/if}
            <input class="downloadBtn large-font" type="submit" value="{t}Download{/t}"/>

            {if $config->convert}
                <div class="mp3 small-font">
                    <div class="mp3-inner">
                        <input type="checkbox" id="audio" class="audio"
                               name="audio" {if $config->defaultAudio}checked{/if}>
                        <label for="audio">
                            <span class="ui"></span>
                            {t}Audio only (MP3){/t}
                        </label>
